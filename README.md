...
**DiyyMusic 0.8.3** (`versionCode 16`)
...
- Fixed `LoginScreen.kt` compilation by removing the invalid explicit Compose `weight` import and using the `ColumnScope` extension correctly.
...
Push the project to the `main` branch and run the **DiyyMusic v0.8.3 APK** workflow. The generated artifact is:

```text
DiyyMusic-v0.8.3-universal.apk
```
